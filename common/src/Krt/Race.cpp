		...
		img.cmd_prim.record([&](auto &cmd){
			...
			cmd.render(img.opaque_fb, {{0, 0}, instance.swapchain->extent()},
				...
				[&](auto &cmd){
					...
					s_sche.cur_cam = view;
					glm::mat4 cur_cam_normal = view;
					for (size_t i = 0; i < 3; i++)
						cur_cam_normal[3][i] = 0.0f;
					s_sche.cur_cam_normal = cur_cam_normal;
					...
					s_sche.last_cam = last_view;
					glm::mat4 last_cam_normal = last_view;
					for (size_t i = 0; i < 3; i++)
						last_cam_normal[3][i] = 0.0f;
					s_sche.last_cam_normal = last_cam_normal;
			...
			img.depth_range->drawDepth(cmd, *m_track);
			...
			img.depth_range->schedule(cmd, img);
